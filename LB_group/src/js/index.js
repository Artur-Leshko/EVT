...
function validateForm({ name, email, password }) {
    ...
    if(name.length <= 2) {
        new_errors.name.push("Имя должно содержать хотя бы 3 символа!");
    }

    if (!/^[а-яА-Я]*$/.test(name)) {
        new_errors.name.push("Имя должно содержать только буквы кириллицы!");
    }
    ...
    if(password.length <= 5) {
        new_errors.password.push("Пароль должен содержать хотя бы 6 символов!");
    }
